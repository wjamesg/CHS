...
def compute_p_values_Main_permutate_y(
    ...
    for _ in range(n):
        ...
        if model_type == "NN":
            runNNFinal(X_train, y_train_shuffled, X_val, y_val_shuffled, dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay,batch_size)
            model = SimpleNN(n_x=X_train.shape[1], dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons,
                             activation_function=activation_function)
            # Jim do I need to send model to device before reading from external file?
            model.to(device)
...
def do_simulation_test(n):
    ...
    for i in range(n):
        ...
        df, ycol, xcols = generate_df(seed) # ycol, xcols provided by simulation function
        feature_dic = {xcols[j]: j for j in range(len(xcols))}
        ...
        # Compute SHAP p-values
        main_p_values_lr = compute_p_values_Main_permutate_y('LR', model_lr, X_train, X_val, X_test, y_train, y_val,
                                                             y_test, ysd_test, yL1_test, xcols, baseline,
                                                             dropout_rate, n_hidden, n_neurons, activation_function,
                                                             lr_init, weight_decay, batch_size, n=100)
        main_p_values_nn = compute_p_values_Main_permutate_y('NN', model, X_train, X_val, X_test, y_train, y_val,
                                                             y_test, ysd_test, yL1_test, xcols, baseline,
                                                             dropout_rate, n_hidden, n_neurons, activation_function,
                                                             lr_init, weight_decay, batch_size, n=100)

        intxn_p_values_lr = {
            "lr_model_intxn_p_g1*e": compute_p_value_Intxn_permutate_y('LR', model_lr, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g1"], feature_dic["e"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "lr_model_intxn_p_g4*z": compute_p_value_Intxn_permutate_y('LR', model_lr, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g4"], feature_dic["z"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "lr_model_intxn_p_g5*z": compute_p_value_Intxn_permutate_y('LR', model_lr, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g5"], feature_dic["z"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "lr_model_intxn_p_g2*e": compute_p_value_Intxn_permutate_y('LR', model_lr, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g2"], feature_dic["e"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "lr_model_intxn_p_g2*z": compute_p_value_Intxn_permutate_y('LR', model_lr, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g2"], feature_dic["z"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
        }
        ...
        intxn_p_values_nn = {
            "nn_model_intxn_p_g1*e": compute_p_value_Intxn_permutate_y('NN', model, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g1"], feature_dic["e"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "nn_model_intxn_p_g4*z": compute_p_value_Intxn_permutate_y('NN', model, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g4"], feature_dic["z"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "nn_model_intxn_p_g5*z": compute_p_value_Intxn_permutate_y('NN', model, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g5"], feature_dic["z"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "nn_model_intxn_p_g2*e": compute_p_value_Intxn_permutate_y('NN', model, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g2"], feature_dic["e"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
            "nn_model_intxn_p_g2*z": compute_p_value_Intxn_permutate_y('NN', model, X_train, X_val, X_test, y_train,
                                                                       y_val, y_test, ysd_test, baseline, feature_dic["g2"], feature_dic["z"],
                                                                       dropout_rate, n_hidden, n_neurons,
                                                                       activation_function, lr_init, weight_decay,
                                                                       batch_size, 100),
        }
        # Reshape main SHAP p-values
        ...
        shap_p_values_iteration.rename(columns={'ShapL2_pval': f'ShapL2_pval_{i}'}, inplace=True)
        ...
        result_row.update(intxn_p_values_lr)
        result_row.update(intxn_p_values_nn)
...
result_summary_df, main_shap_p_values=do_simulation_test(100)
end_time = time.time()
...
main_shap_p_values.to_csv("main_shap_p_values.csv", index=False)