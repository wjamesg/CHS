...
perm_n_epochs            = 200  # how many epochs during final training/testing phase for permutation
...
# Parameter for generate dataframe
n = 10000
...
be = 0.0
bz = 0.0 # continuous E;
bx1 = 0.5 # additive coding of g1;
bex1 = -1.0 # crossing interaction for male*x1 with additive coding for g1;
bx4 = 0.3 # dominant coding of g4;
bx5 = 0.3 # recessive coding of g5;
bzx4 = 0.5 # interaction of z with g4 (dominant);
bzx5 = 0.5 # interaction of z with g5 (recessive);
...
def generate_df(seed):
    ...
    return df, ["y"], ["e", "z"] + [f"g{i + 1}" for i in range(10)]
...
class SimpleNN(nn.Module):
    def __init__(self, n_x, dropout_rate=0.19, n_hidden=2, n_neurons=64, activation_function="tanh",
                 **activation_kwargs):
...
def objective(trial, X_train, y_train, X_val, y_val):
    ...
    model = SimpleNN(n_x = X_train.shape[1], dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons,
...
def runNNFinal(X_train, y_train, X_val, y_val, dropout_rate, n_hidden, n_neurons, activation_function,
           lr_init, weight_decay, batch_size, model_index):
    model = SimpleNN(n_x = X_train.shape[1], dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons,
                 activation_function=activation_function)
    model.to(device)
    num_epochs = perm_n_epochs
    optimizer  = optim.Adam(model.parameters(), lr=lr_init, weight_decay=weight_decay)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_trial_val_loss:
            best_trial_val_loss = val_loss  # Update the best validation loss if the current one is lower
            torch.save(model.state_dict(), 'best_model' + str(model_index) + '.pth')  # Save the best model
...
def writePerformance(perf_LRtest,perf_NNtest,dict,ycol,xcols):
...
def getSHAP_Main(model_type,model,X, y_sd, y_L1_d, xcols, baseline):
    ...
    if model_type == 'LR':
        ...
        for batch_features, _ in X:
            all_features.append(batch_features.cpu().numpy())  # Convert each batch to numpy

        X = np.concatenate(all_features, axis=0)  # Combine all batches into one array
        ...
        result = pd.DataFrame(list_of_tuples, columns=['Feature', 'ShapL1', 'ShapL2'])
...
if tune:
    ...
    study.optimize(lambda trial: objective(trial, X_train, y_train, X_val, y_val), n_trials=t_n_trials)
...
runNNFinal(X_train, y_train, X_val, y_val, dropout_rate, n_hidden, n_neurons, activation_function,
           lr_init,weight_decay,batch_size)
...
writePerformance(perf_LRtest,perf_NNtest,hyperParmDic,ycol,xcols)
...
def compute_p_values_Main_permutate_y(
    ...
    original_L1 = original_shap_values['ShapL1'].values
    original_L2 = original_shap_values['ShapL2'].values
    ...
    for _ in range(n):
        ...
        if _ == 0:
            shuffled_values_L1 = np.expand_dims(shuffled_shap_values['ShapL1'].values, axis=0)
            shuffled_values_L2 = np.expand_dims(shuffled_shap_values['ShapL2'].values, axis=0)
        else:
            shuffled_values_L1 = np.vstack([shuffled_values_L1, shuffled_shap_values['ShapL1'].values])
            shuffled_values_L2 = np.vstack([shuffled_values_L2, shuffled_shap_values['ShapL2'].values])
    ...
    result_df = pd.DataFrame({
        'Feature': features,
        'ShapL1_pval': p_values_L1,
        'ShapL2_pval': p_values_L2
    })
...
def compute_p_values_Main_permutate_y(
        model_type, model, X_train, X_val, X_test, y_train, y_val, y_test, y_sd, y_L1_d, xcols, baseline,
        dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay,batch_size,
        n=100, model_index = 0
):
    test_dataset = CustomDataset(X_test, y_test)
    test_loader = DataLoader(test_dataset, batch_size=batchsize_test, shuffle=False)
    ...
    for _ in range(n):
        ...
        if model_type == "NN":
            runNNFinal(X_train, y_train_shuffled, X_val, y_val_shuffled, dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay,batch_size, model_index = model_index)
            model = SimpleNN(n_x=X_train.shape[1], dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons,
                             activation_function=activation_function)
            # Jim do I need to send model to device before reading from external file?
            model.to(device)
            model.load_state_dict(torch.load('best_model' + str(model_index) + '.pth'))
            model.to(device)
        else:
            if binary_outcome:
        ...
        shuffled_values_L1.append(shuffled_shap_values['ShapL1'].values)
        shuffled_values_L2.append(shuffled_shap_values['ShapL2'].values)

    shuffled_values_L1 = np.array(shuffled_values_L1)
    shuffled_values_L2 = np.array(shuffled_values_L2)

    p_values_L1 = np.sum(shuffled_values_L1 > original_L1, axis=0) / (n + 1)
    p_values_L2 = np.sum(shuffled_values_L2 > original_L2, axis=0) / (n + 1)
...
def compute_p_value_Intxn_permutate_y(
    model_type, model, X_train, X_val, X_test, y_train, y_val, y_test, y_sd, baseline, i, j,
    dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay,batch_size,
    n=100, regularization="L2", model_index = 0
):
    test_dataset = CustomDataset(X_test, y_test)
    test_loader = DataLoader(test_dataset, batch_size=batchsize_test, shuffle=False)
    ...
    shuffled_values = np.array(shuffled_values)
    p_value = np.sum(shuffled_values > original_value) / (n + 1)

    return p_value

def compute_p_values_main_and_interactions(
        model_type, model, X_train, X_val, X_test, y_train, y_val, y_test, y_sd, y_L1_d, xcols, baseline,
        dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay, batch_size,
        interaction_feature_pairs, n=100, regularization="L2", model_index=0):

    test_dataset = CustomDataset(X_test, y_test)
    test_loader = DataLoader(test_dataset, batch_size=batchsize_test, shuffle=False)

    # Compute Original SHAP Values
    print("Computing original SHAP values...")
    original_shap_values = getSHAP_Main(model_type, model, test_loader, y_sd, y_L1_d, xcols, baseline)
    original_L1 = original_shap_values['ShapL1'].values
    original_L2 = original_shap_values['ShapL2'].values

    # Compute original interaction SHAP values
    original_interaction_values = {}
    for i, j in interaction_feature_pairs:
        original_interaction_values[(i, j)] = get_shap_interaction_adjusted(
            i, j, model, model_type, regularization, y_sd, test_loader, baseline, device
        )

    shuffled_values_L1 = []
    shuffled_values_L2 = []
    shuffled_interaction_values = {pair: [] for pair in interaction_feature_pairs}

    # Permutation Process
    print("Starting permutation process...")
    for iteration in range(n):
        print(f"Permutation {iteration + 1}/{n}")

        # Shuffle y_train and y_val
        y_train_shuffled = y_train[torch.randperm(y_train.size(0))]
        y_val_shuffled = y_val[torch.randperm(y_val.size(0))]

        # Re-train model on shuffled data
        if model_type == "NN":
            runNNFinal(X_train, y_train_shuffled, X_val, y_val_shuffled, dropout_rate, n_hidden, n_neurons,
                       activation_function, lr_init, weight_decay, batch_size, model_index=model_index)
            model = SimpleNN(n_x=X_train.shape[1], dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons,
                             activation_function=activation_function)
            model.to(device)
            model.load_state_dict(torch.load(f'best_model{model_index}.pth'))
            model.to(device)
        else:
            if binary_outcome:
                model, _ = runLogistic(X_train, y_train_shuffled, X_val, y_val_shuffled, "train/val")
        ...
        # Compute shuffled SHAP values
        shuffled_shap_values = getSHAP_Main(model_type, model, test_loader, y_sd, y_L1_d, xcols, baseline)
        shuffled_values_L1.append(shuffled_shap_values['ShapL1'].values)
        shuffled_values_L2.append(shuffled_shap_values['ShapL2'].values)

        # Compute shuffled interaction SHAP values
        for i, j in interaction_feature_pairs:
            shuffled_interaction_value = get_shap_interaction_adjusted(
                i, j, model, model_type, regularization, y_sd, test_loader, baseline, device
            )
            shuffled_interaction_values[(i, j)].append(shuffled_interaction_value)
    ...
    shuffled_values_L1 = np.array(shuffled_values_L1)
    shuffled_values_L2 = np.array(shuffled_values_L2)
    shuffled_interaction_values = {pair: np.array(values) for pair, values in shuffled_interaction_values.items()}

    # Compute p-values for main effects
    p_values_L1 = np.sum(shuffled_values_L1 > original_L1, axis=0) / (n + 1)
    p_values_L2 = np.sum(shuffled_values_L2 > original_L2, axis=0) / (n + 1)

    main_p_values_df = pd.DataFrame({
        'Feature': xcols,
        'ShapL1_pval': p_values_L1,
        'ShapL2_pval': p_values_L2
    })

    # Compute p-values for interactions
    interaction_p_values_list = []
    for (i, j), original_value in original_interaction_values.items():
        p_value = np.sum(shuffled_interaction_values[(i, j)] > original_value) / (n + 1)
        interaction_p_values_list.append({
            "Feature 1": xcols[i],
            "Feature 2": xcols[j],
            "Interaction_p_value": p_value
        })

    interaction_p_values_df = pd.DataFrame(interaction_p_values_list)

    return main_p_values_df, interaction_p_values_df



'''
a=compute_p_values_Main_permutate_y('NN',model, X_train, X_val, X_test, y_train, y_val, y_test, ysd_test, yL1_test, xcols, baseline,
                                    dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay,batch_size,
                                    100) # investigate importance of feature 0 and permutate y for 100 times for calculating p-value

b=compute_p_value_Intxn_permutate_y('NN',model, X_train, X_val, X_test, y_train, y_val, y_test, ysd_test, baseline, 0, 1,
                                    dropout_rate, n_hidden, n_neurons, activation_function, lr_init, weight_decay,batch_size,
                                    100, regularization = "L2")# investigate interaction importance between feature 0 and feature 1, and permutate y for 100 times for calculating p-value
'''



def do_simulation_test(n):
    n_hidden = f_n_hidden
    n_neurons = f_n_neurons
    activation_function = f_activation_name
    dropout_rate = f_dropout_rate
    ...
    results = []
    main_shap_p_values = None

    # Define interaction feature pairs (using column indices)
    interaction_feature_pairs = [("g1", "e"), ("g4", "z"), ("g5", "z"), ("g2", "e"), ("g2", "z")]

    for replica_index in range(n):
        seed = int(time.time())  # Generate a new seed based on the clock
        df, ycol, xcols = generate_df(seed)  # Generate dataset
        feature_dic = {xcols[j]: j for j in range(len(xcols))}  # Map feature names to indices

        # Split data into train, validation, and test sets
        train_df, test_df = train_test_split(df, test_size=testSize, shuffle=True, random_state=seed)
        train_df, val_df = train_test_split(train_df, test_size=valSize, shuffle=True, random_state=seed)
        ...
        # Prepare true model for comparison
        X_true_model = sm.add_constant(train_df[["e", "z", "g1"]])
        X_true_model["x4"] = train_df["g4"].apply(lambda g: 1 if g in [1, 2] else 0)
        X_true_model["x5"] = train_df["g5"].apply(lambda g: 1 if g == 2 else 0)
        ...
        # Train models
        model_lr, perf_LRtest = runLinear(X_train, y_train, X_test, y_test, 'train/test')
        model_true = sm.OLS(y_train, X_true_model).fit()
        runNNFinal(X_train, y_train, X_val, y_val, dropout_rate, n_hidden, n_neurons, activation_function, lr_init,
                   weight_decay, batch_size, model_index=replica_index)

        model = SimpleNN(n_x=X_train.shape[1], dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons,
                         activation_function=activation_function)
        model.to(device)
        model.load_state_dict(torch.load(f'best_model{replica_index}.pth'))
        model.to(device)

        # Compute R-squared and MSE
        rsq_lr, mse_lr = perf_LRtest[1], perf_LRtest[2]
        rsq_true = model_true.rsquared
        mse_true = mean_squared_error(y_train, model_true.predict(X_true_model))

        train_dataset = CustomDataset(X_train, y_train)
        train_loader = DataLoader(train_dataset, batch_size=256, shuffle=False)
        perf_NNtrain = getNNPerformance(model, train_loader)
        rsq_nn, mse_nn = perf_NNtrain[1], perf_NNtrain[2]

        num_baseline = min(1000, len(y_train))
        baseline = get_sample(train_loader, num_baseline)

        # Compute SHAP main and interaction p-values
        main_p_values_lr, intxn_p_values_lr = compute_p_values_main_and_interactions(
            model_type='LR', model=model_lr, X_train=X_train, X_val=X_val, X_test=X_test, y_train=y_train, y_val=y_val,
            y_test=y_test, y_sd=ysd_test, y_L1_d=yL1_test, xcols=xcols, baseline=baseline,
            dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons, activation_function=activation_function,
            lr_init=lr_init, weight_decay=weight_decay, batch_size=batch_size,
            interaction_feature_pairs=[(feature_dic[i], feature_dic[j]) for i, j in interaction_feature_pairs],
            n=100, regularization="L2", model_index=replica_index
        )

        main_p_values_nn, intxn_p_values_nn = compute_p_values_main_and_interactions(
            model_type='NN', model=model, X_train=X_train, X_val=X_val, X_test=X_test, y_train=y_train, y_val=y_val,
            y_test=y_test, y_sd=ysd_test, y_L1_d=yL1_test, xcols=xcols, baseline=baseline,
            dropout_rate=dropout_rate, n_hidden=n_hidden, n_neurons=n_neurons, activation_function=activation_function,
            lr_init=lr_init, weight_decay=weight_decay, batch_size=batch_size,
            interaction_feature_pairs=[(feature_dic[i], feature_dic[j]) for i, j in interaction_feature_pairs],
        ...
        # Modify main p-values for feature names
        main_p_values_lr['Feature'] = main_p_values_lr['Feature'] + '_lr'
        main_p_values_nn['Feature'] = main_p_values_nn['Feature'] + '_nn'

        # Merge SHAP p-values for this iteration
        shap_p_values_iteration = pd.concat([main_p_values_lr[['Feature', 'ShapL2_pval']],
                                             main_p_values_nn[['Feature', 'ShapL2_pval']]], axis=0)
        shap_p_values_iteration.rename(columns={'ShapL2_pval': f'ShapL2_pval_{replica_index}'}, inplace=True)

        if main_shap_p_values is None:
            main_shap_p_values = shap_p_values_iteration
        else:
            main_shap_p_values = main_shap_p_values.merge(shap_p_values_iteration, on='Feature', how='left')

        # Merge interaction p-values
        intxn_p_values_lr_dict = {f"lr_model_intxn_p_{i}*{j}": val
                                  for (i, j), val in zip(intxn_p_values_lr[['Feature 1', 'Feature 2']].values,
                                                         intxn_p_values_lr['Interaction_p_value'])}
        intxn_p_values_nn_dict = {f"nn_model_intxn_p_{i}*{j}": val
                                  for (i, j), val in zip(intxn_p_values_nn[['Feature 1', 'Feature 2']].values,
                                                         intxn_p_values_nn['Interaction_p_value'])}

        # Store summary results
        result_row = {
            "lr_model_Rsq": rsq_lr,
            "lr_model_MSE": mse_lr,
            "true_model_Rsq": rsq_true,
            "true_model_MSE": mse_true,
            "nn_model_Rsq": rsq_nn,
            "nn_model_MSE": mse_nn,
        }
        result_row.update(intxn_p_values_lr_dict)
        result_row.update(intxn_p_values_nn_dict)

        results.append(result_row)

    result_summary_df = pd.DataFrame(results)
    return result_summary_df, main_shap_p_values

def do_single_replica(replica_index):
    n_hidden = f_n_hidden
    n_neurons = f_n_neurons
    activation_function = f_activation_name
    dropout_rate = f_dropout_rate
    ...
    seed = int(time.time()) + replica_index  # Generate a new seed based on the clock
    df, ycol, xcols = generate_df(seed)  # Generate dataset
    feature_dic = {xcols[j]: j for j in range(len(xcols))}  # Map feature names to indices
    ...
    # Train models
    model_lr, perf_LRtest = runLinear(X_train, y_train, X_test, y_test, 'train/test')
    model_true = sm.OLS(y_train.cpu().numpy(), X_true_model).fit()
    runNNFinal(X_train, y_train, X_val, y_val, dropout_rate, n_hidden, n_neurons, activation_function, lr_init,
               weight_decay, batch_size, model_index=replica_index)
    ...
    result_summary_df = pd.DataFrame(results)
    result_summary_df.to_csv(f"results/result_summary{replica_index}.csv", index=False)
    main_shap_p_values.to_csv(f"results/main_shap_p_values{replica_index}.csv", index=False)
    return

if __name__ == "__main__":
    replica_index = int(sys.argv[1])  # Get index from command line argument
    do_single_replica(replica_index)
